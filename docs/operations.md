...
For the background development process started during setup, logs are in `.local/server.log` and `.local/server-error.log`; `.local/server.pid` identifies its PowerShell parent. The startup script checks an occupied port, verifies the API identity, health and local credential, and prints the existing explorer URL without rebuilding. If verification fails, it exits with a diagnostic instead of starting another build. To apply code changes, stop the existing API first and rerun the script. Normal use of `start-local.ps1` runs in your terminal and stops with Ctrl+C.
